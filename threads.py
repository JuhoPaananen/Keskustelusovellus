"""This module take care of database methods and functions"""
...
def get_topics(id):
    sql = text("SELECT T.title, U.username, T.created_at FROM topics T LEFT JOIN users U ON T.user_id = U.id WHERE T.category_id = :category_id ORDER BY T.created_at ASC")
    result = db.session.execute(sql, {"category_id": id})
    return result.fetchall()

def save_new_message(content, topic_id, user_id):
    sent_at = datetime.datetime.now()
    sql = text("INSERT INTO messages (user_id, topic_id, content, sent_at) VALUES (:user_id, :topic_id, :content, :sent_at)")
...
def remove_message(message_id, user_id):
    ...
    sql = text("UPDATE messages SET visible=:bool, updated_at=:updated WHERE id=:message_id AND user_id=:user_id")
...
def get_messages(topic_id):
    sql = text("SELECT M.content, U.username, M.sent_at, U.id, M.id \
                FROM messages M \
                LEFT JOIN users U ON M.user_id = U.id WHERE M.topic_id = :topic_id AND visible=:visible \
...
def search_messages(keyword):
    query = "%"+keyword+"%"
    sql = text("SELECT M.content, M.sent_at, M.id \
                FROM messages M \
...
def save_new_topic(category_id, new_topic, content, user_id):
    sql_topic = text("INSERT INTO topics (user_id, category_id, title) VALUES (:user_id, :category_id, :title)")
